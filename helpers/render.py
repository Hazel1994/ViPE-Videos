...
def render_animation(args, anim_args, animation_prompts, root):
    ...
    if args.n_img_reward_samples is not None:
        args.n_samples = args.n_img_reward_samples
    else:
        args.n_samples = 1
    ...
    while frame_idx < anim_args.max_frames:
        ...
        if args.n_samples <2:
            ...
            results = generate(args, root, frame_idx, return_latent=False, return_sample=True)
            samples, images=results[0],results[1:]

            # select the best image out args.n_samples samples
            prompt = args.prompt
            with torch.no_grad():
                ranking, rewards = reward_model.inference_rank(prompt, images)
            image=images[ranking[0] - 1]
            sample=samples[ranking[0] - 1]
